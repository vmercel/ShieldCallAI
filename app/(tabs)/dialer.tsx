...
function VoiceTab({
  ...
  const [statusLine, setStatusLine] = useState('Say Call, then a name');
  ...
  useEffect(() => {
    const t = setTimeout(() => voice.startListening(), 450);
    return () => {
      clearTimeout(t);
      voice.stopListening();
      Speech.stop();
  ...
  const handleCommand = useCallback((cmd: ReturnType<typeof parseVoiceCommand>) => {
    const key = `${cmd.action}:${cmd.target ?? ''}:${cmd.rawText}`;
    if (handledRef.current === key) return;
    handledRef.current = key;

    if (cmd.action === 'ghost') {
      setStatusLine('Ghost Mode — answering for you');
      speak('Ghost Mode. I will answer for you.');
      router.push('/ghost-mode');
      return;
    }
    if (cmd.action === 'cancel') {
      setStatusLine('Cancelled. Say Call, then a name.');
      speak('Cancelled.');
      setMatches([]);
      return;
    }
    if (cmd.action === 'call' && cmd.target) {
      if (cmd.targetType === 'number') {
        setStatusLine(`Calling ${cmd.target}`);
        speak(`Calling ${cmd.target}`);
        onCallNumber(cmd.target);
        return;
      }
      const results = searchContactsSync(cmd.target);
      if (results.length === 1) {
        setMatches([]);
        setStatusLine(`Calling ${results[0].name}`);
        speak(`Calling ${results[0].name}`);
        onCallContact(results[0]);
        return;
      }
      if (results.length > 1) {
        setMatches(results.slice(0, 5));
        setStatusLine(`I found ${results.length}. Who should I call?`);
        speak(`I found ${results.length} matches. Who should I call?`);
        return;
      }
      setMatches([]);
      setStatusLine(`No contact named ${cmd.target}. Try again.`);
      speak(`I could not find ${cmd.target} in your contacts.`);
      setTimeout(() => voice.startListening(), 1600);
      return;
    }
    setStatusLine('Say Call, then a name.');
    speak('Say Call, then a name.');
    setTimeout(() => voice.startListening(), 1200);
  }, [onCallContact, onCallNumber, router, voice]);

  useEffect(() => {
    if (voice.parsedCommand && voice.state === 'done') handleCommand(voice.parsedCommand);
  }, [voice.parsedCommand, voice.state, handleCommand]);
    ...
    <View style={styles.voiceTab}>
      <View style={styles.voiceHero}>
        <Text style={styles.voiceTitle}>
          {isListening ? 'Listening...' : voice.state === 'processing' ? 'Finding contact...' : statusLine}
        </Text>
      ...
      {voice.state === 'error' && voice.errorMessage ? (
        <TouchableOpacity style={styles.voiceError} onPress={handleMic} activeOpacity={0.85}>
          <MaterialIcons name="refresh" size={16} color={Colors.warning} />
          <Text style={styles.voiceErrorText}>{voice.errorMessage}</Text>
        </TouchableOpacity>
      ) : null}
      ...
      {matches.length > 0 && (
        <View style={styles.matchList}>
          {matches.map(c => (
            <TouchableOpacity key={c.id} style={styles.matchRow} onPress={() => onCallContact(c)} activeOpacity={0.85}>
              <ContactAvatar contact={c} size={40} />
              <View style={{ flex: 1 }}>
                <Text style={styles.matchName}>{c.name}</Text>
                <Text style={styles.matchNum}>{c.number}</Text>
              </View>
...
function AgentVoicePrompt({
  ...
  useEffect(() => {
    if (voice.state !== 'done' || !voice.finalText) return;
    if (handledRef.current === voice.finalText) return;
    handledRef.current = voice.finalText;
    onHeard(voice.finalText);
    onRun(voice.finalText);
  }, [voice.state, voice.finalText, onHeard, onRun]);

  const listening = voice.state === 'listening';

  return (
    <View style={styles.agentVoiceWrap}>
      <Text style={styles.agentInstructLabel}>Say what the AI should handle</Text>
      <MicButton isListening={listening} onPress={() => {
        handledRef.current = null;
        if (listening) voice.stopListening();
        else { voice.reset(); voice.startListening(); }
      }} />
      <Text style={styles.agentVoiceHint}>
        {listening
          ? 'Listening... e.g. Call CVS and refill my medication'
          : instruction
          ? instruction
          : 'Tap the mic and speak the task'}
      </Text>
    </View>
  );
}
...
function AIAgentTab() {
  ...
  const executeCallWith = useCallback(async (task: string) => {
    if (!task.trim()) return;
    setInstruction(task);
    setPhase('running');
    setResult(null);
      ...
      {phase === 'idle' && (
        <>
          <AgentVoicePrompt
            ...
            onHeard={(text) => setInstruction(text)}
            onRun={(text) => {
              setInstruction(text);
              setTimeout(() => executeCallWith(text), 50);
            }}
          />
          {instruction ? (
            <TouchableOpacity
              style={styles.executeBtn}
              onPress={() => executeCallWith(instruction)}
              activeOpacity={0.85}
            >
          ...
          <View style={styles.quickTaskGrid}>
            {QUICK_TASKS.map(task => (
              <TouchableOpacity
                ...
                onPress={() => executeCallWith(task.example)}
                activeOpacity={0.8}
              >
                ...
                {result.transcript.map((line, i) => {
                  ...
                  return (
                    <View key={i} style={[
                      ...
                      <Text style={styles.transcriptTextSm}>{line.text}</Text>
                    </View>
                  );
                })}
              </ScrollView>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 12 }]}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Say a name to call</Text>
...
const styles = StyleSheet.create({
  ...
  voiceErrorText: { flex: 1, fontSize: FontSize.sm, color: Colors.warning, lineHeight: 18 },
  matchList: { gap: 8 },
  matchRow: {
    flexDirection: 'row', alignItems: 'center', gap: 10,
    backgroundColor: Colors.bgCard, borderRadius: Radius.md,
    padding: Spacing.sm + 2, borderWidth: 1, borderColor: Colors.borderStrong,
  },
  matchName: { fontSize: FontSize.md, fontWeight: FontWeight.bold, color: Colors.text },
  matchNum: { fontSize: FontSize.xs, color: Colors.textSecondary, marginTop: 2 },
  agentVoiceWrap: { alignItems: 'center', gap: Spacing.md, paddingVertical: Spacing.sm },
  agentVoiceHint: { fontSize: FontSize.sm, color: Colors.textSecondary, textAlign: 'center', lineHeight: 20, paddingHorizontal: Spacing.md },
  ...
  transcriptTextSm: { fontSize: FontSize.sm, color: Colors.text, lineHeight: 19 },
  newTaskBtn: {
